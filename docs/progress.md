...
- **改善・修正タスク完了**: 
  - ✅ タイムゾーン修正（JST対応）
  - ✅ ユーザー名分割（last_name/first_name）
  - ✅ カレンダーUI改善
  - ✅ レイアウト再構成
  - ✅ ランディングページ改善
  - ✅ Analytics設定
- **DDD移行完了**: Phase 1〜7（全52タスク完了）
...
| T011 | 承認機能 | 打刻承認管理 | backend / api | clock_records / store_settings | 中 | [x] | pending / approved / rejected 状態管理 |
| T012 | 打刻編集 | 打刻編集機能（承認制適用） | backend / api | clock_records | 中 | [x] | 管理者が承認制の場合のみ反映 |
| T013 | カレンダー表示 | スタッフ向けカレンダー | UI / api | shifts / clock_records | 高 | [x] | 自分のシフト・過去打刻履歴表示、日ごと色分け、複数店舗切替 |
| T014 | カレンダー表示 | 管理者向けカレンダー | UI / api | shifts / clock_records | 高 | [x] | 店舗全体・個人別勤務状況、日/週/月表示、未打刻者リスト、月ナビゲーションUI改善 |
...
| UI001 | UI改善 | モダンなデザイン適用・共通ナビゲーション | UI | レイアウト | 中 | [x] | デザインのモダン化、全ページでナビゲーション表示、ブルー系テーマカラー適用 |
| UI002 | ランディングページ | ランディングページ作成 | UI | ランディング | 中 | [x] | サービス紹介、機能説明、CTAボタン、モダンなデザイン |

---

## 2. DDD移行タスク一覧

| Phase | ドメイン | タスク数 | 状態 | 備考 |
|-------|---------|---------|------|------|
| Phase 1 | User | 10タスク | [x] | エンティティ、値オブジェクト、リポジトリ、ユースケース、Presentation層接続、動作確認 |
| Phase 2 | Store/Company | 8タスク | [x] | エンティティ、リポジトリ、ユースケース、Presentation層接続、動作確認 |
| Phase 3 | Shift | 7タスク | [x] | エンティティ、値オブジェクト、リポジトリ、ユースケース、Presentation層接続、動作確認 |
| Phase 4 | ClockRecord | 8タスク | [x] | エンティティ、値オブジェクト、ドメインサービス、リポジトリ、ユースケース、Presentation層接続、動作確認 |
| Phase 5 | StoreSettings/Calendar/WorkSummary | 7タスク | [x] | エンティティ、ユースケース、Presentation層接続、動作確認 |
| Phase 6 | Auth | 4タスク | [x] | ドメインサービス、ユースケース、Presentation層接続、動作確認 |
| Phase 7 | ディレクトリ最適化 | 8タスク | [x] | ドメインごとのモジュール化、lib/actions削除、不要ディレクトリ削除 |

**合計**: 52タスク / 52タスク完了 (100%)
